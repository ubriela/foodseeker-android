 ...
 * @author linghu
 */
public class BuildingsOverlay extends ItemizedOverlay<BuildingsOverlayItem> {

	private List<BuildingsOverlayItem> overlays;

	private BuildingsOverlayItem chosenItem;
	private MapView map;
	 ...
	 *            MapView
	 */
	public BuildingsOverlay(Drawable defaultMarker, MapView map, Context con) {
		super(defaultMarker);
		overlays = new ArrayList<BuildingsOverlayItem>();
	...
	protected boolean onTap(int index) {
		...
		BuildingsOverlayItem buildingOverlayItem = overlays.get(index);
		BuildingsDialog buildingDialog = new BuildingsDialog(context,
				buildingOverlayItem.getPlace());
	 ...
	 *            BuildingOverlayItem
	 */
	public void addOverlay(BuildingsOverlayItem overlay) {
	...
	@Override
	protected BuildingsOverlayItem createItem(int i) {
	 ...
	 */
	public void setSelectedItem(BuildingsOverlayItem chosenItem) {
	...
	public void setSelectedItem(int id) {
		...
		for (BuildingsOverlayItem item : overlays) {
	...
	public boolean onTouchEvent(MotionEvent event, MapView mapView) {
		...
		if (MapBrowserActivity.isSpatialQuery) {
			...
			if (event.getAction() == 1) {
				...
				((MapBrowserActivity) context).getMap().getOverlays()
						.add(itemizedoverlay);
				...
				if (MapBrowserActivity.tappedPoints.size() == 4) { // Rectangle,
					...
					if (MapBrowserActivity.queryType == QueryType.Circle
						...
						((MapBrowserActivity) context).getMap().getOverlays()
								.add(shapeOverlay);
						((MapBrowserActivity) context).getMap().invalidate();
